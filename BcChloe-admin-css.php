<?php
/*
Plugin Name: BcChloe Admin Custom CSS
Plugin URI: https://github.com/ifNoob/BcChloe-admin-css
Description: Admin CSS works with Multisite. (Folk Admin CSS MU http://millionclues.com)
Author: BcChloe
Author URI: https://bcchloe.jp
Version: 1.2
Text Domain: admin-custom-css
...
function admincssmu_add_menu_links() {
	if ( is_multisite() ) {
		...
		# origin
//	dd_theme_page ( __('Admin CSS MU','admin-css-mu'), __('Admin CSS MU','admin-css-mu'), 'update_core', 'admin-css-mu','admincssmu_admin_interface_render'  );
		# tools項目部
		add_management_page ( __('Admin CSS','admin-custom-css'), __('Admin CSS','admin-custom-css'), 'update_core', 'admin-custom-css','admincssmu_admin_interface_render'  );
		# 設定項目部
//		add_options_page ( __('Admin CSS','admin-custom-css'), __('Admin CSS','admin-custom-css'), 'update_core', 'admin-custom-css','admincssmu_admin_interface_render'  );
		}
	} else {
		#	origin
...
* 2018/6
-----------------*/
	if ( ! function_exists( 'bc_register_codemirror' ) ) {
		wp_enqueue_style( 'bc-admin-custom-css', plugins_url( basename( dirname( __FILE__ ) ) . '/lib/codemirror.css' ) );																	# default theme
//	wp_enqueue_style( 'bc_admin_custom-theme', plugins_url( basename( dirname( __FILE__ ) ) . '/theme/oceanic-next.css' );															# color scheme style load
//	wp_enqueue_style( 'bc_admin_custom-theme', plugins_url( basename( dirname( __FILE__ ) ) . '/theme/night.css' );																			# color scheme style load
		wp_enqueue_style( 'bc_admin-custom-theme', plugins_url( basename( dirname( __FILE__ ) ) . '/theme/erlang-dark.css' ) );															#
		wp_enqueue_style( 'bc_admin-custom-addon', plugins_url( basename( dirname( __FILE__ ) ) . '/addon/lint/lint.css' ) );																# addon style load
		wp_enqueue_script( 'bc-admin_custom-js', plugins_url( basename( dirname( __FILE__ ) ) . '/lib/codemirror.js' ), array(), '', '' );									# default js
		wp_enqueue_script( 'bc-admin_custom-css-js', plugins_url( basename( dirname( __FILE__ ) ) . '/mode/css/css.js' ), array(), '', '' );								# default.js
		wp_enqueue_script( 'bc-admin_custom-addon-lint', plugins_url( basename( dirname( __FILE__ ) ) . '/addon/lint/lint.js'), array(), '', '' );					# addon line ja
		wp_enqueue_script( 'bc-admin_custom-addon-csslint', plugins_url( basename( dirname( __FILE__ ) ) . '/addon/lint/csslint.js'), array(), '', '' );		#
		wp_enqueue_script( 'bc-admin_custom-addon-css-lint', plugins_url( basename( dirname( __FILE__ ) ) . '/addon/lint/css-lint.js'), array(), '', '' );	#
	}
...
?>
	<div class="wrap">
		<h1><?php _e('Admin Custom CSS','admin-custom-css') ?></h1>
		<p><?php _e('Enter your custom CSS below and it will be loaded on all Admin Pages.','admin-custom-css') ?></p>
		...
		<script language="javascript">
			jQuery( document ).ready( function() {
				var editor_admin_css = CodeMirror.fromTextArea( document.getElementById( "admincssmu_custom_css[admincssmu_admin_css]" ), {
					lineNumbers: true,
					tabSize: 2,
					mode: "css",
//	theme: "oceanic-next",
